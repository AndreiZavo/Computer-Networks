...
int main() {
    ...
    while (1) {
        ...
        int res = recv(c, (char*)&strSize, sizeof(strSize), 0);
        if (res != sizeof(strSize))
            printf("Error receiving operand - size\n");
        
        strSize = ntohl(strSize);

        //recieveBuffer.resize(strSize, 0x00);
        char* buffer;
        buffer = (char*)malloc(sizeof(char) * (strSize+1));

        res = recv(c, buffer, strSize + 1, 0);
